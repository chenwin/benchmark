...
sysinfo()
{
  ...
  for cmd in dmesg env lscpu lsmod lspci lsblk dmidecode free
  do
...
function toMBps()
{
  ...
  unit=$2
  if [ "${unit}" == "iB/s" ];then
    value=$(echo ${value} | awk '{printf ("%.2f",$1*1.048576/1024/1024)}')
  elif [ "${unit}" == "KiB/s" ];then
    value=$(echo ${value} | awk '{printf ("%.2f",$1*1.048576/1024)}')
  elif [ "${unit}" == "GiB/s" ];then
    value=$(echo ${value} | awk '{printf ("%.2f",$1*1.048576*1024)}')
  elif [ "${unit}" == "MiB/s" ];then
    value=$(echo ${value} | awk '{printf ("%.2f",$1*1.048576)}')
...
function parse_fio()
{
 ...
 write_bw_value=$(echo ${write_bw} | awk -F 'MiB/s' '{print $1}')
 write_bw_unit=$(echo ${write_bw} | grep -o 'MiB/s')
